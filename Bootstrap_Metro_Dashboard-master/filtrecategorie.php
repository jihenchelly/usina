...
if(($q=="Filter By Category")&&($p1==0)&&($p2==0))
    $list=$cc->afficheproduit($cc->conn);
	else
	{
		if($q=="Filter By Category")
		
		$list=$cc->filtreselonprix($cc->conn,$p1,$p2);	
	}

    
	
if(($q!="Filter By Category")&&($p1==0)&&($p2==0))     
	$list=$cc->filtreseloncategory($cc->conn,$q);
else
{
	if($q!="Filter By Category")
	$list=$cc->filtreselonprixcategorie($cc->conn,$q,$p1,$p2);	
}
