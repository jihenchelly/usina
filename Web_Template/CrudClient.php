...
class CrudClient{
	...
	function backupClient($Email,$conn){
		...
		$req="SELECT First_Name,Last_Name,Sex,Email,Password,CIN,Street,Region,Birthdate,Phone FROM clients WHERE Email='$Email'";
            	$liste=$conn->query($req);
		return $liste->fetchAll();
	}
	function modifyClient($client,$temp,$conn){
            //$temp= $client->getEmail();
		$req1="UPDATE clients SET First_Name='".$client->getFirstName()."',Last_Name='".$client->getLastName()."',Sex='".$client->getSex()."',Email='".$client->getEmail()."',CIN='".$client->getCIN()."',Street='".$client->getStreet()."',Region='".$client->getRegion()."',Birthdate='".$client->getBirthdate()."',Phone='".$client->getPhone()."' WHERE Email='$temp'";
		$conn->exec($req1);
                echo $req1;
